...
namespace Do_Pham_Alexandre_Meyer_Adrien_Probleme
{
	public class Image 
	{
		...
		/// <summary>
		/// Copy toutes les valeurs d'une matrice de pixel dans une autre
		/// </summary>
		/// <param name="mat"></param>
		public Pixel[,] CopyTotal()
		{
			Pixel[,] matCopy = new Pixel[this._matPixel.GetLength(0), this._matPixel.GetLength(1)];
				...
				ligne++)
			{
				for (int colonne = 0;
					...
					colonne++)
				{
					matCopy[ligne, colonne] = new Pixel(0);
					matCopy[ligne, colonne].Copy(this._matPixel[ligne, colonne]);
				}
			}
			return matCopy;
		}
		...
		public void CopyDonne(Pixel[,] mat, int ligneDepart, int colonneDepart, bool msg = false)
		{
				...
				ligne++)
			{
				for (int colonne  = colonneDepart;
					...
					colonne++)
				{
					bool blanc = PixelBlanc(mat[ligne - ligneDepart, colonne - colonneDepart]);
					if (!msg ||
						!blanc)
					{
		...
		/// <summary>
		/// renvoi true si le pixel en parametre est blanc
		/// </summary>
		/// <param name="pix"></param>
		/// <returns></returns>
		private bool PixelBlanc(Pixel pix)
		{
			bool blanc = true;
			if (pix.Rouge !=255 ||
				pix.Vert !=255 ||
		...
		/// <summary>
		/// color le pixel à la position donné en parametre de couleur donnée en parametre
		/// </summary>
		/// <param name="ligne"></param>
		/// <param name="colonne"></param>
		/// <param name="couleur"></param>
		public void ColoriagePixelSolo(int ligneDebut, int colonneDebut, Pixel couleur, int taille)
		{
			Pixel passage = new Pixel(couleur.Bleu, couleur.Vert, couleur.Rouge); //changement d'emplacement de memoire
			for (int ligne = ligneDebut;
				ligne < ligneDebut+taille;
				ligne++)
			{
				for (int colonne = colonneDebut;
					colonne < colonneDebut + taille;
					colonne++)
				{
					bool possible = Bordure(ligne, colonne);
					if (possible)
					{
						this._matPixel[ligne, colonne] = passage;
					}
				}
			}
		}
		...
		public void Convolution(int[,] matConvo,bool flue = false)
		{
		...
		/// <summary>
		/// Colorisation en rouge d'une image
		/// </summary>
		public void Rouge()
		{
			for (int ligne = 0;
							...
							ligne++)
			{
				for (int colonne = 0;
					...
					colonne++)
				{
					this.MatPixel[ligne, colonne].Rougation();
				}
			}
		}
		/// <summary>
