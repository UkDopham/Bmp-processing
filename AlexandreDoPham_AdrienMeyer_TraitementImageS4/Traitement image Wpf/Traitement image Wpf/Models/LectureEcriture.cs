...
{
	public class LectureEcriture : ObservableObject
	{
		...
		public LectureEcriture(string file)
		{
			Lecture(file);
			CalculPadding();
		...
		public void ModifDirectTab(int lignePos, int colonnePos, Pixel couleur,int taille)
		{
			for (int ligne = 0;
				ligne < taille;
				ligne++)
			{
				for (int colonne = 0;
					...
					colonne++)
				{
					if (ligne < this._hauteur &&
						colonne < this._largeur)
					{
						ModifValeurTabSolo(lignePos + ligne, colonnePos + colonne, couleur);
					}
				}
			}
		}
		...
		/// <summary>
		/// Trouver la postion du tableau à partir d'une coordonnée de la matrice de pixel
		/// </summary>
		/// <param name="ligne"></param>
		/// <param name="colonne"></param>
		/// <returns></returns>
		public int FromMatToByteArray(int ligne, int colonne)
		{

			int ligneInv = this._hauteur - ligne - 1;
			int colonneInv = colonne;
			int index = 54 + ligneInv * this._largeur*3 + 3 * colonneInv + this._padding * ligneInv;
			return index;
		}

		/// <summary>
		/// modifie direct dans le tab sans passer dans la classe image pour opti 
		/// </summary>
		/// <param name="ligne"></param>
		/// <param name="colonne"></param>
		private void ModifValeurTabSolo(int ligne, int colonne, Pixel couleur)
		{
			int index = FromMatToByteArray(ligne, colonne);
			this._fichier[index] = couleur.Bleu;
			this._fichier[index + 1] = couleur.Vert;
			this._fichier[index + 2] = couleur.Rouge;
		}
